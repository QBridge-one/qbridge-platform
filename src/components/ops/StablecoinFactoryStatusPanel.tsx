...
// ============================================================
// components/ops/StablecoinFactoryStatusPanel.tsx
//
// Ops view of the stablecoin factory: live status (paused?, count,
// wired registries/authority, implementation templates) + pause /
// unpause controls. Mirror of FactoryStatusPanel for the stablecoin
// product (different factory address, count label, impl struct).
//
...
import { useContracts, useStablecoinFactoryAddress } from "@/lib/hooks/useContracts";
import {
  useIsPaused,
  useStablecoinCount,
  useImplementations,
  ...
  usePauseFactory,
  useUnpauseFactory,
} from "@/lib/generated/stablecoin-factory";
...
export function StablecoinFactoryStatusPanel() {
  const { chainId } = useContracts();
  const factory = useStablecoinFactoryAddress();

  const isPaused = useIsPaused(undefined);
  const count = useStablecoinCount(undefined);
    ...
    return (
      <Card>
        <CardContent className="p-6 text-sm text-muted-foreground">
          Stablecoin factory isn&apos;t configured for chain {chainId}. Set
          NEXT_PUBLIC_STABLECOIN_FACTORY_* and switch to a supported network.
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            ...
            <CardDescription className="text-xs">StablecoinFactory</CardDescription>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-6">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Stablecoins deployed</p>
              <p className="mt-1 text-lg font-semibold">{total === undefined ? "—" : total.toString()}</p>
            </div>
          </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">Cloned per stablecoin by the factory.</CardDescription>
        </CardHeader>
        <CardContent>
          <dl className="divide-y">
            ...
            <AddressLine label="Reserve oracle" value={i?.reserveOracle} chainId={chainId} />
          </dl>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Pausing halts new stablecoin deployments (createStablecoin reverts) until unpaused.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {paused ? (
